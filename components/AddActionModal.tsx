...
import { Search, Camera, BookOpen, Plus, X, Sparkles, ScanBarcode, MessageSquareText } from "lucide-react";
...
interface AddActionModalProps {
  ...
  onSelectOption: (option: "search" | "ai" | "recipes" | "custom" | "barcode" | "textmeal") => void;
    ...
    <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-1 gap-2.5">
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className={`text-[12px] ${dark ? "text-zinc-400" : "text-gray-500"}`}>
                Ponad 200+ produktów z gramaturą i makro
              </div>
            </div>
          </button>
          ...
          <button
            type="button"
            onClick={() => onSelectOption("barcode")}
            className={`flex items-center gap-3.5 rounded-2xl p-4 border text-left transition-all hover:scale-101 active:scale-99 ${
              dark
                ? "bg-zinc-800/80 border-zinc-700 hover:bg-zinc-800"
          ...
          >
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-orange-500 text-white shadow-md shadow-orange-500/25">
              <ScanBarcode className="h-6 w-6" />
            </div>
            <div className="min-w-0 flex-1">
              <div className="text-[15px] font-bold">Zeskanuj kod kreskowy</div>
              <div className={`text-[12px] ${dark ? "text-zinc-400" : "text-gray-500"}`}>
                Skanuj produkt z opakowania lub wpisz kod EAN
              </div>
            </div>
          </button>

          {/* Opisz posiłek tekstem */}
          <button
            type="button"
            onClick={() => onSelectOption("textmeal")}
            className={`flex items-center gap-3.5 rounded-2xl p-4 border text-left transition-all hover:scale-101 active:scale-99 ${
              dark
                ? "bg-zinc-800/80 border-zinc-700 hover:bg-zinc-800"
          ...
          >
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-teal-500 text-white shadow-md shadow-teal-500/25">
              <MessageSquareText className="h-6 w-6" />
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                <span className="text-[15px] font-bold">Opisz co zjadłeś</span>
                <Sparkles className="h-3.5 w-3.5 text-teal-500" />
              </div>
              <div className={`text-[12px] ${dark ? "text-zinc-400" : "text-gray-500"}`}>
                Wpisz opis posiłku, AI obliczy kalorie i makro
              </div>
            </div>
          </button>
